...
const QUIZZES = {
  vasopressors: { label: "Vasopressor Dosing", icon: "💊", items: [
  ...
  acls: { label: "ACLS Algorithms", icon: "❤️‍🔥", items: [
    ...
    <div style={{ fontFamily: "'DM Sans','Helvetica Neue',sans-serif", background: t.bg, color: t.tx, minHeight: "100vh", transition: "all 0.3s" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 16px", borderBottom: `1px solid ${t.bd}`, background: t.bgC, position: "sticky", top: 0, zIndex: 100, gap: "8px", flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <button onClick={() => { setPg("dash"); setSel(null); setProto(null); }} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px", padding: 0 }}>
            <div style={{ width: "30px", height: "30px", borderRadius: "8px", background: `linear-gradient(135deg,${t.ac},${t.bl})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px" }}>🩺</div>
            <span style={{ fontSize: "16px", fontWeight: 700, color: t.tx }}>CRNA Prep</span>
          </button>
          {pg !== "dash" && <button onClick={() => { setPg("dash"); setSel(null); setProto(null); }} style={{ background: t.bgS, border: `1px solid ${t.bd}`, borderRadius: "6px", padding: "4px 10px", color: t.t2, cursor: "pointer", fontSize: "12px", fontWeight: 500 }}>← {pg === "detail" || pg === "proto" ? "Back" : "Dashboard"}</button>}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "4px", background: t.bgS, borderRadius: "7px", padding: "4px 10px", border: `1px solid ${t.bd}` }}>
            <button onClick={() => setTimerOn(!timerOn)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "12px", padding: 0 }}>{timerOn ? "⏸" : "▶"}</button>
            <span style={{ fontSize: "12px", fontFamily: "monospace", color: t.t2, minWidth: "58px" }}>{fmtTime(timer)}</span>
            {timer > 0 && <button onClick={() => { setTimer(0); setTimerOn(false); }} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "10px", color: t.tM, padding: 0 }}>↺</button>}
          </div>
          <button onClick={() => setSo(true)} style={{ display: "flex", alignItems: "center", gap: "6px", background: t.bgS, border: `1px solid ${t.bd}`, borderRadius: "7px", padding: "5px 10px", color: t.tM, cursor: "pointer", fontSize: "12px" }}>
            🔍 <span style={{ display: "none" }}>{/* mobile hide text */}</span><span style={{ background: t.bgC, padding: "1px 5px", borderRadius: "3px", fontSize: "10px", border: `1px solid ${t.bd}` }}>⌘K</span>
          </button>
          <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} style={{ background: t.bgS, border: `1px solid ${t.bd}`, borderRadius: "7px", width: "32px", height: "32px", cursor: "pointer", fontSize: "14px", display: "flex", alignItems: "center", justifyContent: "center" }}>{theme === "dark" ? "☀️" : "🌙"}</button>
        </div>
      </nav>
      ...
      {so && <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", justifyContent: "center", paddingTop: "10vh" }} onClick={() => setSo(false)}>
        <div style={{ background: t.bgC, borderRadius: "14px", width: "92%", maxWidth: "520px", maxHeight: "440px", overflow: "hidden", border: `1px solid ${t.bd}`, boxShadow: t.sh }} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ maxHeight: "380px", overflowY: "auto", padding: "4px" }}>
            ...
            {fs.protos.map(p => <SearchRow key={p.id} icon="❤️‍🔥" title={p.name} sub={p.cat} stars={conf[p.id]} onClick={() => oPro(p)} t={t} />)}
      ...
      {pg === "dash" && <div style={{ maxWidth: "1060px", margin: "0 auto", padding: "24px 16px" }}>
        ...
        {vm === "type" && <>
          ...
          <div style={{ marginBottom: "20px" }}><PH t={t} text="Ask about Frank-Starling, MAC, O₂-Hb curve..." /></div>
        ...
        {/* SYSTEM VIEW — ACLS has its own row, NOT under Cardio */}
        {vm === "system" && <>
          {/* ACLS — standalone system row */}
          <SL t={t} icon="❤️‍🔥" title="ACLS & PALS Protocols" count={PROTOS.length} color="#ef4444" />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: "10px", marginBottom: "20px" }}>
            {PROTOS.map(p => <ProtoCard key={p.id} p={p} t={t} conf={conf[p.id]} onConf={v => setConf(prev => ({ ...prev, [p.id]: v }))} onOpen={() => oPro(p)} />)}
          </div>
      ...
      {pg === "pg-meds" && <div style={{ maxWidth: "1060px", margin: "0 auto", padding: "24px 16px" }}>
        ...
        <SL t={t} icon="📖" title="Physiology Concepts" count={0} />
        <div style={{ marginBottom: "24px" }}><PH t={t} text="Ask about Frank-Starling, MAC, O₂-Hb curve..." /></div>
      </div>}
      ...
      {pg === "qres" && <div style={{ maxWidth: "420px", margin: "0 auto", padding: "56px 16px", textAlign: "center" }}>
        <div style={{ fontSize: "44px", marginBottom: "12px" }}>{qSc.c / qSc.t >= 0.8 ? "🎉" : "📚"}</div>
    ...
    <div style={{ maxWidth: "920px", margin: "0 auto", padding: "20px 16px" }}>
      ...
      {tab === "diagram" && <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px", marginBottom: "14px" }}>
          <div><h3 style={{ color: t.ac, fontSize: "17px", margin: "0 0 3px" }}>{item.id === "norepinephrine" ? "α₁/β₁/α₂ Adrenergic Receptor Pathways" : item.id === "vasopressin" ? "V₁a / V₂ / KATP Channel Pathways" : "GABA-A Receptor Diagram"}</h3>
            <p style={{ color: t.tM, margin: 0, fontSize: "12px" }}>{item.id === "norepinephrine" ? "NE binding → Gq/Gs/Gi cascades → vasoconstriction + inotropy + autoregulation" : item.id === "vasopressin" ? "AVP binding → V₁a/Gq vasoconstriction + KATP closure + V₂/Gs antidiuresis" : "Propofol binding → Cl⁻ influx → hyperpolarization"}</p></div>
